...
let pendingImport = null; // { tempPath, skillName } for import conflict resolution
...
if (typeof require !== 'undefined' && typeof require.config === 'function') {
  ...
  require(['vs/editor/editor.main'], function () {
    ...
    // Custom theme matching modern glassmorphism UI
...
const importConflictName = document.getElementById('importConflictName');
...
const importRenameModal = document.getElementById('importRenameModal');
const importRenameInput = document.getElementById('importRenameInput');
const importRenameCancelBtn = document.getElementById('importRenameCancelBtn');
const importRenameConfirmBtn = document.getElementById('importRenameConfirmBtn');
...
const modalConfirmPairs = [
  ...
  { modal: importRenameModal, confirmButton: importRenameConfirmBtn }
];
...
importSkillBtn.addEventListener('click', async () => {
  ...
  const result = await window.electronAPI.openSkillFileDialog();
  if (!result.success) return;

  const importResult = await window.electronAPI.importSkillPackage({
    filePath: result.filePath
  });

  if (!importResult.success) {
    showNotification('Failed to import: ' + (importResult.error || 'Unknown error'), 'error');
    return;
  }

  if (importResult.conflict) {
    // Store pending import info and show conflict modal
    pendingImport = { tempPath: importResult.tempPath, skillName: importResult.skillName };
    importConflictName.textContent = importResult.skillName;
    importConflictModal.classList.add('active');
  } else {
    // No conflict, import completed
    showNotification(`Imported "${importResult.skillName}" successfully`, 'success');
    const skills = await loadSkills();
    // Auto-select the imported skill
    const importedSkill = skills.find(s => s.name === importResult.skillName);
    if (importedSkill) {
      await loadSkill(importedSkill);
    }
  }
...
importOverwriteBtn.addEventListener('click', async () => {
  ...
  importConflictModal.classList.remove('active');

  const result = await window.electronAPI.confirmImportSkill({
    tempPath: pendingImport.tempPath,
    skillName: pendingImport.skillName,
    action: 'overwrite'
  });

  if (result.success) {
    showNotification(`Imported "${result.skillName}" successfully (overwrote existing)`, 'success');
    const skills = await loadSkills();
    const importedSkill = skills.find(s => s.name === result.skillName);
    if (importedSkill) {
      await loadSkill(importedSkill);
    }
  } else {
...
importRenameBtn.addEventListener('click', () => {
  if (!pendingImport) return;

  importConflictModal.classList.remove('active');
  // Suggest a new name by appending -2
  importRenameInput.value = pendingImport.skillName + '-2';
  importRenameModal.classList.add('active');
  importRenameInput.focus();
  importRenameInput.select();
});
...
importCancelBtn.addEventListener('click', async () => {
  ...
  if (pendingImport) {
    // Clean up temp files
    await window.electronAPI.confirmImportSkill({
      tempPath: pendingImport.tempPath,
      action: 'cancel'
    });
    pendingImport = null;
...
// Import rename modal handlers
importRenameCancelBtn.addEventListener('click', async () => {
  importRenameModal.classList.remove('active');
...
importRenameConfirmBtn.addEventListener('click', async () => {
  if (!pendingImport) return;

  const newName = importRenameInput.value.trim();
  ...
  if (!/^[a-z0-9-]+$/.test(newName)) {
    showNotification('Skill name can only contain lowercase letters, numbers, and hyphens', 'error');
    return;
  }
  ...
  if (result.success) {
    showNotification(`Imported "${result.skillName}" successfully`, 'success');
    const skills = await loadSkills();
    const importedSkill = skills.find(s => s.name === result.skillName);
    if (importedSkill) {
      await loadSkill(importedSkill);
...
// Prevent default drag behavior on document to enable custom drop handling
document.addEventListener('dragover', (e) => {
  e.preventDefault();
});
document.addEventListener('drop', (e) => {
  console.log('DROP on document (fallback)', e.dataTransfer.files);
  e.preventDefault();
});
...
editorArea.addEventListener('dragover', (e) => {
  ...
  // Always show drop zone for potential .skill file imports, or if a skill is selected
  if (!isDraggingOverFolder && !anyFolderHasDragOver) {
...
// Handle drop on editor area (in case drop zone doesn't catch it)
editorArea.addEventListener('drop', async (e) => {
  console.log('DROP on editorArea', e.dataTransfer.files);
  e.preventDefault();
  e.stopPropagation();
  uploadDropZone.classList.remove('active');
  uploadDropZone.style.display = 'none';

  // Delegate to the common drop handler
...
uploadDropZone.addEventListener('drop', async (e) => {
  console.log('DROP on uploadDropZone', e.dataTransfer.files);
  e.preventDefault();
  e.stopPropagation();
  uploadDropZone.classList.remove('active');
  uploadDropZone.style.display = 'none';
...
// Common file drop handler
async function handleFileDrop(e) {
  const items = e.dataTransfer.items;
  const files = e.dataTransfer.files;
  if (!items && !files) return;

  // Check if any dropped file is a .skill or .zip file
  const droppedFiles = Array.from(files || []);
  const skillFile = droppedFiles.find(f =>
    f.name.endsWith('.skill') || f.name.endsWith('.zip')
  );

  if (skillFile) {
    // Handle as skill import
    await handleSkillFileDrop(skillFile);
    return;
  }

  // Otherwise handle as regular file upload (requires a skill to be selected)
  if (!currentSkill) {
    showNotification('Select a skill first, or drop a .skill file to import', 'error');
    return;
  }
...
// Handle .skill file drop for import
