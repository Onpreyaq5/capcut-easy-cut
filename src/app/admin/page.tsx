      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-surface">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-xs"><thead className="bg-surface-muted text-text-secondary"><tr><Th>บัญชี</Th><Th>สถานะ</Th><Th>แพ็กเกจ</Th><Th>ใช้ไป/ลิมิต</Th><Th>รายได้</Th><Th>เข้าใช้</Th><Th>จัดการ</Th></tr></thead><tbody>
          {stats.users.map((user) => <tr key={user.email} className="border-t border-border/60 align-top">
            ...
            <Td><select value={user.plan} disabled={busy === user.email} onChange={(e) => post('/api/auth/admin/setplan', { email: user.email, plan: e.target.value }, user.email)} className="min-h-11 rounded-lg border border-border bg-background px-2 text-text-primary"><option value="free">Free</option><option value="pro">Pro</option><option value="studio">Studio</option></select></Td>
            <Td><div className="flex items-center gap-2"><input type="number" min="0" defaultValue={Math.round(user.usedSeconds / 60)} id={`usage-${user.email}`} className="h-11 w-20 rounded-lg border border-border bg-background px-2 text-text-primary" /><button onClick={() => { const el = document.getElementById(`usage-${user.email}`) as HTMLInputElement; post('/api/auth/admin/manage', { action: 'setUsage', email: user.email, usedMinutes: Number(el.value) }, `usage-${user.email}`); }} className="min-h-11 rounded-lg bg-primary-soft px-3 font-bold text-primary">บันทึก</button></div><p className="mt-1 text-text-muted">ลิมิต {Math.round(user.limitSeconds / 60)} นาที</p></Td>
